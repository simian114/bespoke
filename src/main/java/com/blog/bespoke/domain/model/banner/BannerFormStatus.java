 ...
 * - 결제취소(PAYMENT_CANCEL): 결제 대기 중 상태에서 취소 된 상태다. 원상태로의 복구가 불가능하다. 처음부터 심사를 다시 받아야한다. 게시전까지 결제하지 않으면 이 상태가 된다.
 ...
 * - 게시종료(END): 게시중인 광고가 스케줄러에 의해 자동으로 게시종료가 된다. 정상적으로 게시 되고 정상적으로 게시 마무리 되면 되는 상태다.
 *
 *
 * 완전 종료 상태는 아래와 같다.
 *   - DENIED
